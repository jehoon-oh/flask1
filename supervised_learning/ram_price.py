...
class RamPrice:
    ...
    def execute(self):
        ...
        x_all = ram_price['date'].values.reshape(-1, 1)  # x_all을 1열로 변환

        pred_tree = tree.predict(x_all)
        price_tree = np.exp(pred_tree)  # log값 되돌리기
        pred_lr = lr.predict(x_all)
        price_lr = np.exp(pred_lr)  # log값 되돌리기
        plt.semilogy(ram_price['date'], pred_tree, label='tree_predict', ls='-', dashes=(2, 1))
        plt.semilogy(data_train['date'], data_train['price'], label='train_data', alpha=0.4)
